/***************************************************
此程序为12系列单片机产生PWM  ，这里未给出配置程序 大家自己配置吧。
PWM调速，PID控制基本稳定 。具体效果大家自己体会。至此，分享给大家。
PID算法，进行整形简化，运算速度提高，精度不算高，但是还行，基本稳定在 +-30转左右。
*****************************************************/
#include"main.h"
...
/*以下部分代码是讲所有的正数项叠加，负数项叠加*/
	...
	if( PostSum > NegSum )             // 是否控制量为正数
	{
		...
		if(Temp[0] < SPEED_MAX)   //小于限幅值则为计算值输出，PWM与调节量的匹配过程
	    {
			PID.Uk = (uint)Temp[0];
		...
		{
			PID.Uk = SPEED_MAX;    //否则为限幅值输出,PWM最大调节范围；PWM控制电机转速，不能小于50%占空比
		}
		PID.Uk_flag = 0;
	}
	else               //控制量输出为负数，则输出0
