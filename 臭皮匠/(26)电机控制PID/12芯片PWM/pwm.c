...
uint time  = T5ms;      //5ms时的CCAP0L和CCAP0H值 uint test  = 0;         //测试用,要删除 
uint test;
sbit LED0 = P3^3;        //用来观测定时模式和捕获模式的现象 
...
{ 
    ...
    CCAP0H = 0xff;   //当CL值大于CCAP0L时输出为高,反之输出为低 

                      //当CL溢出时CCAP0H的值装载到CCAP0L中 
	CCAP1L = 0xff;   //%50占空比 

    CCAP1H = 0xff;   //当CL值大于CCAP0L时输出为高,反之输出为低 
