...
* 【语    言】：C语言
* 【编译环境】：Keil4  
* 【程序功能】：1. 通过本例程了解74HC165（并入串出） 的基本原理和使用 
				2.了解掌握SPI总线接口的工作原理及一般编程方法
				3.连接方法：用8PIN排线将JP5与JP6连接 和 JP11（P2）与JP1（LED灯）连接
* 【技术支持】：74HC165（并入串出）				
* 【晶    振】: 12MHz外部晶振		
* 【芯    片】: STC89C52RC 
* 【日    期】: 2013/6/30
  ...
  { 
    unsigned long  Input=ReHC74165();//调用165驱动程序
     ...
     P2 = 0xff;	  //清除
	  ...
	  //实现脉冲输入  大家仔细体会
    if(K0&K0J)M0=~M0;      
	   K0J=~K0;  		     //经典的按键，扫描的过程中 K0 高， KOJ 低
...
{  
  ...
  unsigned int indata;
   ...
   NOP(); 	 //此处165已经读到了数据，（IN_PL上升沿是一个开关）
